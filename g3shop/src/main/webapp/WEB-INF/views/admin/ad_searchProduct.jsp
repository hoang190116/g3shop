    ...
    <div class="card-header">
        ...
        <div class="table-responsive p-0">
            ...
            <nav aria-label="Page navigation example">
                <input type="hidden" name="search" value="${searchName}">
                <ul class="pagination justify-content-center">
                    <li class="page-item">
                        <c:if test="${currentP > 1}">
                        <form method="get" action="<c:if test="${currentP > 1}">${pageContext.request.contextPath }/adm/searchProduct/${currentP - 1}</c:if>">
                            <button class="page-link" aria-label="Previous" name="search" value="${searchName}" type="submit">
                    ...
                    <c:forEach var="i" begin="1" end="${countP}">
                        <li class="page-item <c:if test="${currentP == i}">active</c:if>"><form method="get" action="${pageContext.request.contextPath }/adm/searchProduct/${i}"><button class="page-link" name="search" value="${searchName}" type="submit">${i}</button></form></li>
                    </c:forEach>
                    <li class="page-item">
                        <c:if test="${currentP < countP}">
                        <form method="get" action="<c:if test="${currentP < countP}">${pageContext.request.contextPath }/adm/searchProduct/${currentP + 1}</c:if>">
                        <button class="page-link" aria-label="Next" name="search" value="${searchName}" type="submit">
