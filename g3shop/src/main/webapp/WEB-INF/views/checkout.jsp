...
<!-- Central Modal Small -->
      ...
      <form class="needs-validation" novalidate>
        ...
        <div class="mb-3">
          ...
          <span id="pCode-input">
          <input class="form-control" id="postalCode" required onKeyUp="if(this.value>999999999){this.value='99999999';}else if(this.value<1){this.value='1';}">
          </span>
        </div>
        <hr class="mb-4">
        <c:if test="${payment == true}">
        ...
        <a class="btn btn-primary btn-lg btn-block" id="pay-button">Buy</a>
            ...
            <script>
                $('#fname-input').on("change", "#fullname",function(){
                 ...
                 $('#pCode-input').on("change", "#postalCode",function(){
                    var adres = $('#postalCode').val();
                    if(adres){
                        $("#pCode-input").html("<input value='"+adres+"' type='number' class='form-control' id='postalCode' onKeyUp='if(this.value>99999999){this.value="+''+99999999+''+";}else if(this.value<1){this.value="+''+1+''+";}'>");
                    }else{
                        $("#pCode-input").html("<input style='border: 2px solid red' class='form-control' id='postalCode' type='number' onKeyUp='if(this.value>99999999){this.value="+''+99999999+''+";}else if(this.value<1){this.value="+''+1+''+";}'>");
                    }
                 });
                ...
                $("#pay-button").click(function(event){
                    ...
                    var pCode = Number($('#postalCode').val());
                    if(!pCode){
                        $("#pCode-input").html("<input style='border: 2px solid red' class='form-control' id='postalCode'>");
                    }
                    var country = $('#country').val();
                    if(!country){
                        $("#country-input").html("<input style='border: 2px solid red' class='form-control' id='country' placeholder='Your country/nation'>");
                    ...
                    if(${payment} === true){
                    if(fullname && email && adres && country && nameCard && numberCard && mmCard && yyCard && cvvCard && pCode){
                            ...
                            if(saveInfo){
                                ...
                                $.ajax({
                                    ...
                                    data:{fname:fullname, email:email, adres:adres, country:country, 
                                        ...
                                        cvvCard:cvvCard, bankName: bankName, total:totalPrice, idP:idP, postalCode:pCode},
                                    url:'savingpaying',
                    ...
                    }else if(fullname && email && adres && country && pCode){
                        var agreebox = $('#agreePolicy:checked').val();
                        if(agreebox){
                        var totalPrice = Number($('#totalInputHidden').val());
                        var idP = Number(${idP});
                                $.ajax({
                                    type:'POST',
                                    data:{fname:fullname, email:email, adres:adres, country:country, total:totalPrice, idP:idP, postalCode:pCode},
                                    url:'Cashpaying',
                                    success:function(result){
                                    }
                                });
